...
import { __DEV__ } from '@agile-solid/utils';
...
const rules: Rule<AgileTheme>[] = [
  ...
  withAutocomplete$(match('col-(span)-(\\d+)', 'gridColumn', span), __DEV__ && (() => range({ end: 12 }))),
  ...
  withAutocomplete$(match('grid-rows-(\\d+)', 'gridTemplateRows', gridTemplate), __DEV__ && (() => range({ end: 6 }))),

  // Grid Row Start / End
  matchTheme('row-', 'gridRow'),
  withAutocomplete$(match('row-(span)-(\\d+)', 'gridRow', span), __DEV__ && (() => range({ end: 6 }))),
  ...
  matchTheme('-?space-(x|y)(?:$|-)', 'space', ({ 1: $1, _ }) => ({
    '&>:not([hidden])~:not([hidden])': {
      ...
      ['margin-' + { y: 'top', x: 'left' }[$1 as 'y' | 'x']]: `calc(${_} * calc(1 - var(--tw-space-${$1}-reverse)))`,
      ['margin-' + { y: 'bottom', x: 'right' }[$1 as 'y' | 'x']]: `calc(${_} * var(--tw-space-${$1}-reverse))`,
    },
  })),
  ...
  withAutocomplete$(match('list-', 'listStyleType'), __DEV__ && (() => ['none', 'disc', 'decimal'])),
  ...
  withAutocomplete$(
    ...
    __DEV__ && (() => ['baseline', 'top', 'middle', 'bottom', 'text-top', 'text-bottom', 'sub', 'super'])
  ),
  ...
  // Background Clip
  match('bg-clip-(border|padding|content|text)', 'backgroundClip', ({ 1: $1 }) => $1 + ($1 == 'text' ? '' : '-box')),
    ...
    __DEV__ &&
      (({ 1: $1 }) =>
        $1.endsWith('-inside-')
          ...
          : /* before || after */ ['auto', 'avoid', 'all', 'avoid-page', 'page', 'left', 'right', 'column'])
  ),
  ...
  // Resize
  ['resize(?:-(none|x|y))?', 'resize', ({ 1: $1 }) => ({ x: 'horizontal', y: 'vertical' }[$1] || $1 || 'both')],
...
function join(value: ThemeMatchResult<MaybeArray<string>> | MaybeArray<string> | undefined): string | undefined {
...
function range({ start = 1, end, step = 1 }: { start?: number; end: number; step?: number }): string[] {
  const result: string[] = [];
